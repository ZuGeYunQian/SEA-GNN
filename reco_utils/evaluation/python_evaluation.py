...
def mrr_at_k(
    ...
    Returns:
        float: MRR at k (min=0, max=1).
    """

    df_hit, _, n_users = merge_ranking_true_pred(
        rating_true=rating_true,
        rating_pred=rating_pred,
        col_user=col_user,
        col_item=col_item,
    ...
    # Calculate the reciprocal rank for hit items
    df_mrr = df_hit.copy()
    df_mrr["rr"] = 1 / df_mrr["rank"]

    # Compute the mean reciprocal rank
    return df_mrr["rr"].sum() / n_users
