...
def create_hparams(flags):
    ...
    init_dict = {
        ...
        "wordEmb_file": None,
        "entityEmb_file": None,
        "contextEmb_file": None,
        "news_feature_file": None,
        ...
        "cross_l2": 0.0000,
        "cross_l1": 0.0000,
        "attn_loss_weight": 0.0000,
        "discrepancy_loss_weight": 0.0000,
        "contrastive_loss_weight": 0.0000,
...
def mrr_at_k_score(y_true, y_score, k=10):
    """Computing MRR@k score metric.
    ...
    non_zero_indices = np.where(rr_score != 0)[0]
    ...
    for metric in metrics:
        if metric == "auc":
            if np.isnan(np.sum(labels)):
                print("预测结果中存在 NaN 值，请检查模型或数据。")
                nan_positions_labels = np.isnan(labels)
                print("NaN 在 labels 中的位置：", np.where(nan_positions_labels))
            if np.isnan(np.sum(preds)):
                print("预测结果中存在 NaN 值，请检查模型或数据。")
                nan_positions_preds = np.isnan(preds)
                print("NaN 在 preds 中的位置：", np.where(nan_positions_preds))
        ...
        elif metric == "mrr_at_10":
            mrr_at_10 = np.mean(
                [
                    mrr_at_k_score(each_labels, each_preds, k=10)
                    for each_labels, each_preds in zip(labels, preds)
                ]
            )
            res["mrr_at_10"] = round(mrr_at_10, 4)
