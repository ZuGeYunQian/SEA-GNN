...
class BaseModel:
    def __init__(self, hparams, iterator_creator, graph=None, seed=None):
        ...
        if hparams.train_num_ngs is not None:
            self.train_num_ngs = hparams.train_num_ngs
        else:
            self.train_num_ngs = None
        ...
        with self.graph.as_default():
            ...
            self.embedding_items1, self.embedding_items2, self.logit = self._build_graph()  # ZGYQ_debug
    ...
    def _get_initializer(self):
        if self.hparams.init_method == "tnormal":
            ...
            return tf.compat.v1.keras.initializers.VarianceScaling(uniform=False, seed=self.seed)
        elif self.hparams.init_method == "xavier_uniform":
            return tf.compat.v1.keras.initializers.VarianceScaling(uniform=True, seed=self.seed)
    ...
    def _compute_data_loss(self):
        if self.hparams.loss == "cross_entropy_loss":
            ...
            data_loss = -group * tf.reduce_mean(tf.math.log(pos_softmax + 1e-10))
        else:
    ...
    def train(self, sess, feed_dict):
        ...
        return sess.run(
            [
                ...
                self.loss,
                self.data_loss,
                self.embedding_items1,
                self.embedding_items2  # ZGYQ_debug
                # self.merged 改
                #  self.pp1,
                #  self.pp2,
                #  self.pp3,
    ...
    def fit(self, train_file, valid_file, test_file=None):
        ...
        if self.hparams.write_tfevents:
            ...
            tf.profiler.experimental.start(self.hparams.SUMMARIES_DIR)
