...
class SURGEModel(SequentialBaseModel):
    ...
    def _build_seq_graph(self):
        ...
        with tf.name_scope('interest_graph'):
            ...
            for i in range(self.metric_heads):
                ...
                self.weighted_tensor = tf.compat.v1.layers.dense(tf.ones([1, 1]), X.shape.as_list()[-1], use_bias=False)
            ...
            num_edges = tf.cast(tf.compat.v1.count_nonzero(S, [1,2]), tf.float32) # B
        ...
        with tf.name_scope('prediction'):
            ...
            if 'kwai' in socket.gethostname():
                sorted_mask_index = tf.compat.v1.contrib.framework.argsort(self.mask, direction='DESCENDING', stable=True, axis=-1) # B*L -> B*L
                sorted_mask = tf.compat.v1.contrib.framework.sort(self.mask, direction='DESCENDING', axis=-1) # B*L -> B*L
            else:
            ...
            X = tf.compat.v1.batch_gather(X, sorted_mask_index) # B*L*F  < B*L = B*L*F
    ...
    def _interest_fusion_extraction(self, X, A, layer, reuse):
        ...
        with tf.name_scope('interest_extraction'):
            ...
            if 'kwai' in socket.gethostname():
                sorted_score = tf.compat.v1.contrib.framework.sort(cluster_score, direction='DESCENDING', axis=-1) # B*L
