...
def _transpose_batch_time(x):
  ...
  """
    ...
    static_a = tf.zeros([2500, 50, 40])
    # if x_static_shape[1] is None:  # 初始化时，因X第一二维度为空，所以给予值
    #     x_static_shape = static_a.get_shape()
    # dim0 = x.shape[0].value if x.shape[0] is not None else 10  # 给定一个固定的值作为第一维度的大小
    # dim1 = x.shape[1].value if x.shape[1] is not None else 50  # 给定一个固定的值作为第二维度的大小
    # x_static_shape = tf.TensorShape([dim0, dim1]).concatenate(x.get_shape()[2:])
