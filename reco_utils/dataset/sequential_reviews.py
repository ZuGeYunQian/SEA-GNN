...
def data_preprocessing_predict(
        reviews_file,
        meta_file,
        train_file,
        valid_file,
    ...
    reviews_output, meta_output = ml_1m_main(reviews_file)
    ...
    preprocessed_output = _data_processing_ml_1m_final(sampled_instance_file)
...
def data_preprocessing(
    ...
    """
    # reviews_file包含uid', 'iid', 'ts'，meta_file包含'iid', 'category'
    if dataset == 'amazon':
        ...
        reviews_output, meta_output = ml_1m_main(reviews_file)
    elif dataset == 'ml_1m_final':
        reviews_output, meta_output = ml_1m_main(reviews_file)
    elif dataset == 'ml_1m_final_cut':
        reviews_output, meta_output = ml_1m_main_cut(reviews_file)
    elif dataset == 'ml_1m_general':
        reviews_output, meta_output = ml_1m_main_general(reviews_file)
    elif dataset == 'ml_1m_8_1_1':
        reviews_output, meta_output = ml_1m_main(reviews_file)
    elif dataset == 'ml_1m_original':
        reviews_output, meta_output = ml_1m_original_main(reviews_file)
    elif dataset == 'ml_1m_original_ng500':
        reviews_output, meta_output = ml_1m_original_main(reviews_file)
    elif dataset == 'ml_1m_pop':
        reviews_output, meta_output = ml_1m_original_main(reviews_file)
    elif dataset == 'ml_1m_rate_filter':
        reviews_output, meta_output = ml_20m_DCRec_main(reviews_file)
    elif dataset == 'ml_20m_DCRec':
        reviews_output, meta_output = ml_20m_DCRec_main(reviews_file)
    elif dataset == 'ml_20m_DCRec_uni':
        reviews_output, meta_output = ml_20m_DCRec_main(reviews_file)
    elif dataset == 'yelp_global':
        reviews_output, meta_output = yelp_main(reviews_file, meta_file)
    elif dataset == 'taobao_global':
        reviews_output, meta_output = taobao_main(reviews_file)
    elif dataset == 'taobao_final':
        reviews_output, meta_output = taobao_main(reviews_file)
    ...
    if dataset == 'kuaishou':
        ...
        preprocessed_output = _data_processing_taobao_final(sampled_instance_file)
    elif dataset == 'ml_1m':
        preprocessed_output = _data_processing_ml_1m(sampled_instance_file)
    elif dataset == 'ml_1m_8_1_1':
        preprocessed_output = _data_processing_ml_1m_8_1_1(sampled_instance_file)
    elif dataset == 'ml_1m_final':
        preprocessed_output = _data_processing_ml_1m_final(sampled_instance_file)
    elif dataset == 'ml_1m_final_cut':
        preprocessed_output = _data_processing_ml_1m_final(sampled_instance_file)
        ...
        preprocessed_output, alias_file = _data_processing_ml_20m_DCRec(sampled_instance_file)
    elif dataset == 'ml_20m_DCRec':
        preprocessed_output, alias_file = _data_processing_ml_20m_DCRec(sampled_instance_file)
    elif dataset == 'ml_20m_DCRec_uni':
        preprocessed_output = _data_processing_ml_20m_DCRec_uni(sampled_instance_file)
    elif dataset == 'yelp_global':
    ...
    _create_vocab(dataset, train_file, user_vocab, item_vocab, cate_vocab)
    if dataset == 'ml_20m_DCRec':
        _negative_sampling_popularity(
            valid_file, test_file, alias_file, valid_num_ngs, test_num_ngs
        )
    elif dataset == 'ml_1m_pop':
        _negative_sampling_popularity(
            valid_file, test_file, alias_file, valid_num_ngs, test_num_ngs
...
def _negative_sampling_popularity(
        valid_file, test_file, alias_file, valid_neg_nums=4, test_neg_nums=49
):
    with open(alias_file, "r") as file:
        content = file.read()

    alias_file_item_ids = [int(item_id) for item_id in content.strip().split('\t')]
    global item2cate
    # Build alias table for popularity-based negative sampling
    prob, alias_table = _build_alias_table(alias_file_item_ids)
    ...
    for line in valid_lines:
        ...
        neg_items = set()
        while count < valid_neg_nums:
            neg_item = _pop_sampling(alias_table, 1, prob)[0]
    ...
    for line in test_lines:
        ...
        while count < test_neg_nums:
            ...
            count += 1
            neg_items.add(neg_item)
            neg_items_test_all.append(neg_item)
            words[0] = "0"
            words[2] = str(neg_item)
            words[3] = str(item2cate[neg_item])
            write_test.write("\t".join(words) + "\n")
    element_count = Counter(neg_items_test_all)
    # 使用 sorted 函数对统计结果按照出现次数从大到小排序
...
def _build_alias_table(candidates_list):
    # 把用户交互的最后一项和倒数第二项全部提取出来进行_build_alias_table
    prob = dict(Counter(candidates_list))
    alias = prob.copy()
    sorted_dict = dict(sorted(prob.items(), key=lambda item: item[1], reverse=True))
    value_counts = Counter(sorted_dict.values())

    # Print the value and its corresponding count
    ...
    for i in prob:
        alias[i] = -1
        prob[i] = prob[i] / len(candidates_list) * len(prob)
        if prob[i] > 1:
            large_q.append(i)
        elif prob[i] < 1:
            small_q.append(i)
...
def _pop_sampling(alias_table, sample_num, prob_global):
    keys = list(prob_global.keys())
    ...
    for idx, prob in zip(random_index_list, random_prob_list):
        if prob_global[keys[idx]] > prob:
            final_random_list.append(keys[idx])
        else:
            final_random_list.append(alias_table[keys[idx]])
...
def _create_instance(reviews_file, meta_file):
    ...
    user_dict = {}
    item_list = []
    # user_dict 用户id为key 值为 uid \t iid \t ts
...
def _data_processing_taobao_final(input_file):
    # 1 uid iid ts cate
    logger.info("start data processing...")
    dirs, _ = os.path.split(input_file)
    output_file = os.path.join(dirs, "preprocessed_output")
    ...
    for line in f_input:
        line = line.strip()
        user_things = line.split("\t")
        user_id = user_things[1]
        if user_id not in user_dict:
            user_dict[user_id] = []
        user_dict[user_id].append(line)
    for user_behavior in user_dict:
        behavior_list = user_dict[user_behavior]
        num_behavior = len(behavior_list)
        test_split = num_behavior - 1
        valid_split = num_behavior - 2
        for i in range(num_behavior):
            line = behavior_list[i]
            if i < valid_split:
                f_output.write("train" + "\t" + line + "\n")
            elif i < test_split:
                f_output.write("valid" + "\t" + line + "\n")
...
def _data_processing_ml_20m_DCRec(input_file):
    # 1 uid iid ts cate
    logger.info("start data processing...")
    dirs, _ = os.path.split(input_file)
    output_file = os.path.join(dirs, "preprocessed_output")
    alias_table_file = os.path.join(dirs, "alias_table")
    f_alias = open(alias_table_file, "w")
    f_input = open(input_file, "r")
    f_output = open(output_file, "w")
    user_dict = {}
    for line in f_input:
    ...
    for user_behavior in user_dict:
        ...
        for i in range(num_behavior):
            ...
            if is_final_two:
                ...
                alias_table_item_id = split_data[2]
                f_alias.write(alias_table_item_id + "\t")
    f_input.close()
    f_output.close()
    return output_file, alias_table_file

def _data_processing_ml_20m_DCRec_uni(input_file):
    # 1 uid iid ts cate
    logger.info("start data processing...")
    dirs, _ = os.path.split(input_file)
    output_file = os.path.join(dirs, "preprocessed_output")
...
def _data_processing_ml_1m_final(input_file):
    # 1 uid iid ts cate
    logger.info("start data processing...")
    dirs, _ = os.path.split(input_file)
    output_file = os.path.join(dirs, "preprocessed_output")
...
def _data_processing_ml_1m_8_1_1(input_file):
    # 1 uid iid ts cate
    logger.info("start data processing...")
    dirs, _ = os.path.split(input_file)
    output_file = os.path.join(dirs, "preprocessed_output")
    ...
    for user_behavior in user_dict:
        behavior_list = user_dict[user_behavior]
        num_behavior = len(behavior_list)
        train_split = int(num_behavior * 0.8)
        valid_split = int(num_behavior * 0.9)
        for i in range(num_behavior):
            line = behavior_list[i]
            if i < train_split:
                f_output.write("train" + "\t" + line + "\n")
...
def _data_processing_ml_1m(input_file):
    # 1 uid iid ts cate
    logger.info("start data processing...")
    dirs, _ = os.path.split(input_file)
    output_file = os.path.join(dirs, "preprocessed_output")
    ...
    for user_behavior in user_dict:
        behavior_list = user_dict[user_behavior]
        num_behavior = len(behavior_list)
        train_split = int(num_behavior * 0.7)
        valid_split = int(num_behavior * 0.8)
...
def taobao_main(reviews_file):
    """
    读取文件，文件格式为csv，包含五列数据：用户ID(uid)、商品ID(iid)、商品类别(category)、用户行为(behavior)、时间戳(ts)。
对用户行为进行过滤，只保留浏览(pv)行为的记录。
删除重复的用户和商品记录。
对商品类别进行过滤，只保留一个商品对应一个类别的记录，删除一个商品对应多个类别的记录。
对时间戳进行过滤，只保留2017年11月25日到12月3日之间的记录。
对用户进行下采样，将所有用户的记录数量降至原来的5%。
进行k-core过滤，保留至少有10个用户和10个商品的记录。
将处理后的记录分别保存到两个csv文件中，一个用于后续推荐系统的训练，一个用于商品的元数据。
    """
    ...
    reviews['uid'] = reviews['uid'].astype(int)
    reviews['iid'] = reviews['iid'].astype(int)
    reviews['category'] = reviews['category'].astype(int)
    reviews['ts'] = reviews['ts'].astype(int)
    reviews = reviews.sort_values(by=['uid', 'ts', 'iid'])
    k_core = 10
...
def ml_20m_DCRec_main(reviews_file):
    """
    读取文件，文件格式为csv，包含五列数据：用户ID(uid)、商品ID(iid)、商品类别(category)、用户行为(behavior)、时间戳(ts)。
    """
    reviews = pd.read_csv(reviews_file, header=None, names=['uid', 'iid', 'category', 'behavior', 'ts'], low_memory=False, sep='\t')
    reviews = reviews.drop(reviews.index[0])
    ...
    reviews_output = os.path.join(dirs, 'ml_20m_review_recommenders.csv')
    meta_output = os.path.join(dirs, 'ml_20m_business_recommenders.csv')
...
def ml_1m_original_main(reviews_file):
    """
    读取文件，文件格式为csv，包含五列数据：用户ID(uid)、商品ID(iid)、商品类别(category)、用户行为(behavior)、时间戳(ts)。
    """
    reviews = pd.read_csv(reviews_file, header=None, names=['uid', 'iid', 'category', 'behavior', 'ts'])
    ...
    reviews = filter_k_core(reviews, k_core, 'iid', 'uid')
    reviews = filter_k_core(reviews, k_core, 'uid', 'iid')
    unique_user = reviews['uid'].unique()
    unique_item = reviews['iid'].unique()
    unique_category = reviews['category'].unique()
    print("inter: ", len(reviews))
    print("uid: ", len(unique_user))
    print("iid: ", len(unique_item))
    ...
    reviews_output = os.path.join(dirs, 'ml_1m_review_recommenders.csv')
    meta_output = os.path.join(dirs, 'ml_1m_business_recommenders.csv')
...
def ml_1m_main_general(reviews_file):
    """
    读取文件，文件格式为csv，包含五列数据：用户ID(uid)、商品ID(iid)、商品类别(category)、用户行为(behavior)、时间戳(ts)。
    文件经过处理，使评分标准为3及以上的数据为pv，对category进行映射至1-n
    """
    reviews = pd.read_csv(reviews_file, header=None, names=['uid', 'iid', 'category', 'behavior', 'ts'])
    reviews = reviews.drop_duplicates(subset=['uid', 'iid'])
    reviews = filter_items_with_multiple_cids(reviews)
...
def filter_earlier_quarter(df):
    # 获取每个用户的数据条数，计算保留后25%的数量
    user_data_count = df.groupby('uid').size().reset_index(name='data_count')
    user_data_count['filter_count'] = user_data_count['data_count'] * 0.5

    # 合并数据框，保留每个用户后75%的数据
    df_sorted = df.sort_values(by=['uid', 'ts'])  # 按照用户和时间戳排序
    df_filtered = pd.merge(df_sorted, user_data_count[['uid', 'filter_count']], on='uid', how='inner')
    df_filtered = df_filtered.groupby('uid').apply(lambda x: x.tail(int(x['filter_count'].iloc[0])))

    # 重置索引并删除多余的列
    df_filtered = df_filtered.reset_index(drop=True).drop(columns=['filter_count'])

    return df_filtered

def ml_1m_main(reviews_file):
    """
    读取文件，文件格式为csv，包含五列数据：用户ID(uid)、商品ID(iid)、商品类别(category)、用户行为(behavior)、时间戳(ts)。
    文件经过处理，使评分标准为3及以上的数据为pv，对category进行映射至1-n
    """
    reviews = pd.read_csv(reviews_file, header=None, names=['uid', 'iid', 'category', 'behavior', 'ts'], sep='\t')
    reviews = reviews.drop_duplicates(subset=['uid', 'iid'])
    reviews = filter_items_with_multiple_cids(reviews)
    # start_ts = int(datetime.timestamp(datetime(2000, 4, 26, 0, 0, 0)))
    # end_ts = int(datetime.timestamp(datetime(2001, 12, 31, 23, 59, 59)))
    # reviews = reviews[reviews['ts'] >= start_ts]
    # reviews = reviews[reviews['ts'] <= end_ts]
    reviews = reviews.sort_values(by=['uid', 'ts'])
    # user_data_count1 = reviews.groupby('uid').size().reset_index(name='data_count')
    # reviews = filter_earlier_quarter(reviews)
    # user_data_count2 = reviews.groupby('uid').size().reset_index(name='data_count')
    k_core = 5
    reviews = filter_k_core(reviews, k_core, 'iid', 'uid')
    reviews = filter_k_core(reviews, k_core, 'uid', 'iid')
...
def ml_1m_main_cut(reviews_file):
    """
    读取文件，文件格式为csv，包含五列数据：用户ID(uid)、商品ID(iid)、商品类别(category)、用户行为(behavior)、时间戳(ts)。
    文件经过处理，使评分标准为3及以上的数据为pv，对category进行映射至1-n
    """
