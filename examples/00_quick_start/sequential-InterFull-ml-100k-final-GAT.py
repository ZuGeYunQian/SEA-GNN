...
if 'kwai' in socket.gethostname():
    ...
    flags.DEFINE_string('name', 'ml_100k_final-InterFull_GAT', 'Experiment name.')
    #  flags.DEFINE_string('dataset', 'taobao', 'Dataset name.')
    flags.DEFINE_string('dataset', 'ml_100k_final', 'Dataset name.')
    #  flags.DEFINE_string('dataset', 'amazon', 'Dataset name.')
    flags.DEFINE_integer('gpu_id', 4, 'GPU ID.')
    ...
    flags.DEFINE_integer('batch_size', 200, 'Batch size.') # 改 500
    ...
    flags.DEFINE_string('model', 'InterFull_GAT', 'Model name.')
    flags.DEFINE_float('embed_l2', 1e-7, 'L2 regulation for embeddings.')
    flags.DEFINE_float('layer_l2', 1e-7, 'L2 regulation for layers.')  # 改
...
flags.DEFINE_string('test_counterfactual_mode', 'shuffle',
                    'Mode for counterfactual evaluation, could be original, shuffle or recent.')
...
flags.DEFINE_boolean('predict_long_short', True,
                     'Predict whether the next interaction is driven by long-term interest or short-term interest.')
...
# flags.DEFINE_integer('early_stop', 1000, 'Patience for early stop.') # GPU
...
flags.DEFINE_string('data_path', os.path.join("..", "..", "tests", "resources", "deeprec", "sequential"),
                    'Data file path.')
if socket.gethostname() == 'rl3':
    #  flags.DEFINE_string('save_path', '/data/changjianxin/ls-recommenders/saves/', 'Save path.')  #  for step visual
    flags.DEFINE_string('save_path', '/data/changjianxin/ls-recommenders/saves_epoch/',
                        'Save path.')  # for epoch visual
else:
...
flags.DEFINE_float('discrepancy_loss_weight', 0.01,
                   'Loss weight for discrepancy between long and short term user embedding.')
...
def get_model(flags_obj, model_path, summary_path, pretrain_path, finetune_path, user_vocab, item_vocab, cate_vocab,
              train_num_ngs, data_path):
    ...
    if flags_obj.dataset == 'kuaishou':
        ...
        pairwise_metrics = ['mean_mrr', 'ndcg@2;4;6;10', 'hit@2;4;6', 'group_auc', 'mrr_at_10']
        weighted_metrics = ['wauc']
        max_seq_length = 50
        time_unit = 's'
    ...
    # SURGE
        ...
        model = InterModel(hparams, input_creator, seed=RANDOM_SEED)

    elif flags_obj.model == 'InterFull_GAT':
        yaml_file = '../../reco_utils/recommender/deeprec/config/gcn.yaml'
        hparams = prepare_hparams(yaml_file,
                                  embed_l2=flags_obj.embed_l2,
                                  layer_l2=flags_obj.layer_l2,
...
class MailMaster(object):
    ...
    def __init__(self, flags_obj):
        ...
        if 'kwai' in socket.gethostname():
            ...
            self.metrics = ['auc', 'wauc', 'mean_mrr', 'ndcg@2', 'ndcg@4', 'ndcg@6', 'ndcg@10','hit@2', 'hit@4', 'hit@6',
                            'mean_alpha']
    ...
    def send_mail(self, flags_obj, mode, res):
        ...
        if mode in ['test', 'train']:
            ...
            if flags_obj.dataset in ['taobao', 'taobao_global']:
                ...
                """.format(mode, flags_obj.test_counterfactual_mode, flags_obj.name, table_head, table_res_strong_last,
                           table_head, table_res_strong_first, table_head, table_res_weak)
            if flags_obj.dataset == 'kuaishou_open':
                ...
                """.format(mode, flags_obj.test_counterfactual_mode, flags_obj.name,
                           ...
                           table_head, table_res_strong_follow_last, table_head, table_res_strong_follow_first,
                           table_head, table_res_weak)
        else:
...
def main(argv):
    ...
    if record:
        ...
        # 检查文件是否存在，不存在则创建文件 加
        file_path = os.path.join("..", "..", "{}-output.txt".format(flags_obj.name))
        ...
        # file_path = os.path.join("..", "..", "error.txt")
        # if not os.path.exists(file_path):
        #     with open(file_path, "w") as f:
        #         f.write("Test error sample\n\n")
        #         f.flush()
        #
        # with open(file_path, "a") as f:
        #     f.write("Start time: {}\n".format(start_time))
        #     f.close()
        #
        # file_path = os.path.join("..", "..", "correct.txt")
        # if not os.path.exists(file_path):
        #     with open(file_path, "w") as f:
        #         f.write("Correct sample\n\n")
        #         f.flush()
        #
        # with open(file_path, "a") as f:
        #     f.write("Start time: {}\n".format(start_time))
    ...
    setproctitle.setproctitle('{}-Rec'.format(flags_obj.name))
    ...
    elif flags_obj.dataset == 'ml_1m':
        reviews_name = 'data.csv'
        meta_name = ''
        strong_behavior_name = 'UserBuy.csv'
    elif flags_obj.dataset == 'ml_1m_final':
        reviews_name = 'data.csv'
        meta_name = ''
        strong_behavior_name = 'UserBuy.csv'
    elif flags_obj.dataset == 'ml_100k_final':
        reviews_name = 'data.csv'
        meta_name = ''
        strong_behavior_name = 'UserBuy.csv'
    # for test
    ...
    if not os.path.exists(train_file):
        data_preprocessing(*input_files, sample_rate=sample_rate, valid_num_ngs=valid_num_ngs,
    ...
    if flags_obj.dataset in ['taobao', 'taobao_global', 'kuaishou_open','ml_1m','ml_1m_final','ml_100k_final']:
        if flags_obj.dataset in ['taobao', 'taobao_global','ml_1m','ml_1m_final','ml_100k_final']:
            ...
            if not os.path.exists(strong_last_test_file) or not os.path.exists(strong_first_test_file):
                raw_data = (
                    strong_last_test_file, strong_first_test_file, weak_test_file, strong_last_vocab,
                    strong_first_vocab)
            ...
            if not os.path.exists(strong_like_last_test_file) or not os.path.exists(strong_like_first_test_file) \
                    or not os.path.exists(strong_follow_last_test_file) or not os.path.exists(
                strong_follow_first_test_file):
                raw_data = (strong_like_last_test_file, strong_like_first_test_file, strong_follow_last_test_file,
                            strong_follow_first_test_file, weak_test_file,
                            strong_like_last_vocab, strong_like_first_vocab, strong_follow_last_vocab,
                            strong_follow_first_vocab)
                strong_data_preprocessing(raw_data, test_file, strong_file, user_vocab, item_vocab,
                                          test_num_ngs=test_num_ngs, dataset=flags_obj.dataset,
    ...
    model = get_model(flags_obj, model_path, summary_path, pretrain_path, finetune_path, user_vocab, item_vocab,
                      cate_vocab, train_num_ngs, data_path)
    ...
    if flags_obj.test_counterfactual:
        ...
        res_weak = model.run_weighted_eval(weak_test_file, num_ngs=test_num_ngs,
                                           calc_mean_alpha=calc_mean_alpha)  # test_num_ngs is the number of negative lines after each positive line in your test_file
        print(res_weak)

        if flags_obj.dataset in ['taobao', 'taobao_global','ml_1m','ml_1m_final','ml_100k_final']:
            print('strong last interaction:')
            res_strong_last = model.run_weighted_eval(strong_last_test_file, num_ngs=test_num_ngs,
                                                      calc_mean_alpha=calc_mean_alpha)  # test_num_ngs is the number of negative lines after each positive line in your test_file
            print(res_strong_last)
            print('strong first interaction:')
            res_strong_first = model.run_weighted_eval(strong_first_test_file, num_ngs=test_num_ngs,
                                                       calc_mean_alpha=calc_mean_alpha)  # test_num_ngs is the number of negative lines after each positive line in your test_file
            ...
            res_strong_like_last = model.run_weighted_eval(strong_like_last_test_file, num_ngs=test_num_ngs,
                                                           calc_mean_alpha=calc_mean_alpha)  # test_num_ngs is the number of negative lines after each positive line in your test_file
            print(res_strong_like_last)
            print('strong like first interaction:')
            res_strong_like_first = model.run_weighted_eval(strong_like_first_test_file, num_ngs=test_num_ngs,
                                                            calc_mean_alpha=calc_mean_alpha)  # test_num_ngs is the number of negative lines after each positive line in your test_file
            print(res_strong_like_first)
            print('strong follow last interaction:')
            res_strong_follow_last = model.run_weighted_eval(strong_follow_last_test_file, num_ngs=test_num_ngs,
                                                             calc_mean_alpha=calc_mean_alpha)  # test_num_ngs is the number of negative lines after each positive line in your test_file
            print(res_strong_follow_last)
            print('strong follow first interaction:')
            res_strong_follow_first = model.run_weighted_eval(strong_follow_first_test_file, num_ngs=test_num_ngs,
                                                              calc_mean_alpha=calc_mean_alpha)  # test_num_ngs is the number of negative lines after each positive line in your test_file
            print(res_strong_follow_first)

            if flags_obj.enable_mail_service:
                mail_master.send_mail(flags_obj, 'counterfactual',
                                      [res_strong_like_last, res_strong_like_first, res_strong_follow_last,
                                       res_strong_follow_first, res_weak])
    ...
    if flags_obj.only_test:
        ckpt_path = tf.train.latest_checkpoint(model_path)
        model.load_model(ckpt_path)
        res_syn = model.run_weighted_eval(test_file,
                                          num_ngs=test_num_ngs)  # test_num_ngs is the number of negative lines after each positive line in your test_file
    ...
    if flags_obj.dataset in ['kuaishou', 'kuaishou_open', 'taobao_global', 'yelp_global','ml_1m','ml_1m_final','ml_100k_final']:
        eval_metric = 'wauc'
    else:
        eval_metric = 'group_auc'

    num_runs = 10  # Number of times you want to run the experiment

    for run in range(num_runs):
        start_time = time.time()
        model = model.fit(train_file, valid_file, valid_num_ngs=valid_num_ngs, eval_metric=eval_metric, vm=vm, tb=tb,
                          pretrain=flags_obj.pretrain)
        end_time = time.time()
        cost_time = end_time - start_time
        print(f'Run {run + 1}/{num_runs} - Time cost for training is {cost_time / 60.0:.2f} mins')

        # ckpt_path = '../../saves/InterFull_GAT/ml_100k_final-InterFull_GAT/model/epoch_1'

        ckpt_path = tf.train.latest_checkpoint(model_path)
        model.load_model(ckpt_path)

        res_syn = model.run_weighted_eval(test_file, num_ngs=test_num_ngs)
        print(f'Run {run + 1}/{num_runs} Results:')
        print(res_syn)

        file_path = os.path.join("..", "..", "{}-output.txt".format(flags_obj.name))
        with open(file_path, "a") as f:
            f.write(flags_obj.name)
            f.write(str(res_syn))
            f.write("\n\n\n")
            f.close()

        sender = ExperimentEmailSender()
        sender.send_experiment_results(
            to_addr='2549639382@qq.com',
            results=res_syn,
            experiment_name=f'序列推荐模型训练 - Run {run + 1}'
        )
