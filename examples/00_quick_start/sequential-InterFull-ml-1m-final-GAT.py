...
import sys
sys.path.append("/home/zgyq/ZGYQ/SEA-GNN")
...
if 'kwai' in socket.gethostname():
    ...
    flags.DEFINE_string('name', 'ml_1m_final-InterFull_GAT', 'Experiment name.')
    #  flags.DEFINE_string('dataset', 'taobao', 'Dataset name.')
    flags.DEFINE_string('dataset', 'ml_1m_final', 'Dataset name.')
    ...
    flags.DEFINE_float('embed_l2', 1e-6, 'L2 regulation for embeddings.')
    flags.DEFINE_float('layer_l2', 1e-6, 'L2 regulation for layers.')  # 改
...
def main(argv):
    ...
    if flags_obj.dataset in ['taobao', 'taobao_global', 'kuaishou_open','ml_1m','ml_1m_final']:
        if flags_obj.dataset in ['taobao', 'taobao_global','ml_1m','ml_1m_final']:
    ...
    if flags_obj.dataset in ['kuaishou', 'kuaishou_open', 'taobao_global', 'yelp_global','ml_1m','ml_1m_final']:
        eval_metric = 'wauc'
    else:
    ...
    # ckpt_path = tf.train.latest_checkpoint(model_path)
    ckpt_path = '../../saves/InterFull_GAT/ml_1m_final-InterFull_GAT/model/epoch_20'
    # ckpt_path = '../../saves/SURGE/ml_1m-SURGE/model/epoch_11'
    model.load_model(ckpt_path)  # '../../saves/SURGE/taobao-SURGE/model/epoch_3'

    res_syn = model.run_weighted_eval(test_file, num_ngs=test_num_ngs)
    # res_syn = model.run_weighted_eval(test_file + '_group' + '1' + '_test', num_ngs=test_num_ngs)
    print(flags_obj.name)
    print(res_syn)
    ...
    with open(file_path, "a") as f:
        ...
        f.write(str(res_syn))
        f.write("\n")
        f.write("Time cost for training is {0:.2f} mins".format((cost_time) / 60.0))
        f.write("\n\n\n")
        f.close()
    ...
    #     to_addr='',
    #     results=res_syn,
    #     experiment_name='序列推荐模型训练'  # 可选，默认为"实验结果"
    # )
    # for g in [1, 2, 3, 4, 5]:
    #     res_syn_group = model.run_weighted_eval(test_file + '_group' + str(g), num_ngs=test_num_ngs)
    #     print(flags_obj.name + '_group' + str(g))
